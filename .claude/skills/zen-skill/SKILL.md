...
- **Routing — which vehicle for a "brainy consult"**: prefer `clink --cli-name <x> --model <flagship>` (opus / top Grok / etc.) — it runs the flagship on the CLI's **subscription login**, no API cost, and returns only the conclusion. Fall back to native `zen chat/thinkdeep --model <id>` (API-billed) only when you need control clink can't pass through: an extended-**thinking budget** (`thinkdeep --thinking-budget`), or a provider with no subscription login (gemini authenticates by API key). Reserve plain `clink` (config-pinned model) for agentic delegation where a mid-tier model is fine.
